...
class Runge_Kutta_two_steps(Euler_explicit_scheme):
    ...
    def solver(self, t) -> None:
        ...
        for i in range(self.M + 1):
            self.u1[i+2] = self.u0[i+2] - self.DT/self.DX*(self.u0[i+2] - self.u0[i+1])
        self.call_BC(self.u1)
        for i in range(self.M + 1):
            self.u2[i+2] = self.u1[i+2] - self.DT/self.DX*(self.u1[i+2] - self.u1[i+1])

        self.u = 0.5 * (self.u2 + self.u)
        self.call_BC(self.u)
...
def main():
    ...
    plt.savefig("Runge_Kutta_two_steps.png")  
    plt.show()
